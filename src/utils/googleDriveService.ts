...
// Token persists in localStorage - user only needs to authorize ONCE
 ...
 * Token is cached in localStorage so user only authorizes once
 */
export async function uploadToGoogleDrive(
  ...
  console.log('[GoogleDrive] Starting upload with OAuth (cached token)');
  console.log('[GoogleDrive] File:', fileName, 'Size:', blob.size, 'bytes');

  try {
    // Get access token (uses cached token from localStorage if available)
