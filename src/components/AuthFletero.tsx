...
interface AuthFleteroProps {
  onAuthenticated: (fleteroName: string) => void;
  onBack: () => void;
}
...
const AuthFletero: React.FC<AuthFleteroProps> = ({ onAuthenticated, onBack }) => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('auth_fletero_blocked');
    if (stored) {
      const blockedTime = parseInt(stored);
      if (Date.now() < blockedTime) {
        setBlockedUntil(blockedTime);
      } else {
        localStorage.removeItem('auth_fletero_blocked');
        localStorage.removeItem('auth_fletero_attempts');
      }
    }

    const storedAttempts = localStorage.getItem('auth_fletero_attempts');
    if (storedAttempts) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await sheetsApi.authenticateFletero(codigo.trim());

      if (result.authenticated && result.fleteroName) {
        // Reset attempts on success
        localStorage.removeItem('auth_fletero_attempts');
        localStorage.removeItem('auth_fletero_blocked');
        setAttempts(0);

        console.log('[AuthFletero] ✅ Authentication successful:', result.fleteroName);
        onAuthenticated(result.fleteroName);
      } else {
        // Increment attempts
        const newAttempts = attempts + 1;
        setAttempts(newAttempts);
        localStorage.setItem('auth_fletero_attempts', newAttempts.toString());

        if (newAttempts >= MAX_ATTEMPTS) {
          // Block user
          const blockUntilTime = Date.now() + BLOCK_DURATION_MS;
          setBlockedUntil(blockUntilTime);
          localStorage.setItem('auth_fletero_blocked', blockUntilTime.toString());
      ...
      console.error('[AuthFletero] Error:', error);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="Ej: XYZ2024AB"
                disabled={loading || !!isBlocked}
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all text-lg uppercase disabled:bg-gray-100 disabled:cursor-not-allowed"
                maxLength={20}
              />
            ...
            {error && (
              <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded space-y-3">
                ...
                <a
                  href="https://wa.me/541173603954?text=Hola%20CROSSLOG%2C%20necesito%20ayuda%20con%20la%20consulta%20de%20fletero"
                  target="_blank"
            ...
            {isBlocked && remainingTime > 0 && (
              <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded space-y-3">
                ...
                <a
                  href="https://wa.me/541173603954?text=Hola%20CROSSLOG%2C%20mi%20acceso%20de%20fletero%20fue%20bloqueado"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg text-sm font-semibold transition-all shadow-md hover:shadow-lg"
                >
            ...
            <button
              ...
              className="w-full py-4 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-xl"
            >
              {loading ? (
