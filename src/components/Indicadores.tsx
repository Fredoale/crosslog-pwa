...
const Indicadores: React.FC<IndicadoresProps> = ({ onClose }) => {
  ...
  // Estados para VALORES DIARIOS DE DISTRIBUCIÓN
  const [dataValoresDiarios, setDataValoresDiarios] = useState<any>(null);
  const [filtroTipoTransporteVD, setFiltroTipoTransporteVD] = useState<string>('todos');
  const [mostrarSoloActivos, setMostrarSoloActivos] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // Load monthly data
    loadDataMensual();
    // Load valores diarios (último mes disponible)
    loadValoresDiarios();
  }, []);
  ...
  const loadValoresDiarios = async (mes?: string) => {
    try {
      console.log('[Indicadores] Cargando valores diarios para mes:', mes);
      const data = await sheetsApi.getValoresDiariosDistribucion(mes);
      setDataValoresDiarios(data);
      return data;
    } catch (error) {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #1a2332 0%, #2d3e50 100%)' }}>
      <div className="bg-white rounded-2xl shadow-2xl p-4 md:p-8 w-full max-w-7xl mx-auto">
        ...
        <ValoresDiariosChart
          data={dataValoresDiarios}
          filtroTipoTransporte={filtroTipoTransporteVD}
