  ...
  const handleFinalizar = async () => {
    ...
    try {
      ...
      const pdfPromises = fotos.map(async (foto, index) => {
        console.log(`[CapturaForm] Preparing image ${index + 1}/${fotos.length} for remito:`, foto.numeroRemito);

        // TEMPORARY: Upload photos directly instead of PDFs
        // PDFs will be generated by N8N workflow
        console.log(`[CapturaForm] ✓ Image ${index + 1} prepared (PDF generation handled by N8N)`);

        return {
          pdf: foto.blob, // Upload photo blob directly
      ...
      const uploadPromises = pdfResults.map(async ({ pdf, numeroRemito }, index) => {
        const filename = `${numeroRemito}.jpg`;
        console.log(`[CapturaForm] Uploading image ${index + 1}/${pdfResults.length}: ${filename}`);
