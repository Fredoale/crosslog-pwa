...
interface DetalleChoferDocumentosProps {
  nombreChofer: string;
  onBack: () => void;
  onAgregarNuevo: (datosPreCargados?: { unidad?: string; tipoUnidad?: string; habilidad?: string; dni?: string }) => void;
}

export function DetalleChoferDocumentos({ nombreChofer, onBack, onAgregarNuevo }: DetalleChoferDocumentosProps) {
  ...
  const cargarDocumentos = async () => {
    setCargando(true);
    try {
      const docs = await sheetsApi.fetchChoferDocumentos(nombreChofer);
      setDocumentos(docs);
      if (docs.length > 0) {
        setUnidad(docs[0].unidad || '');
        setTipoUnidad(docs[0].tipoUnidad || '');
        setHabilidad(docs[0].habilidad || '');
        setDni(docs[0].dni || '');
      }
    } catch (error) {
      console.error('[DetalleChofer] Error:', error);
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleGuardarEdicion = async () => {
    ...
    setGuardando(true);
    try {
      await sheetsApi.updateChoferDocumento(documentoEditar);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="text-white p-6" style={{ background: 'linear-gradient(135deg, #1a2332 0%, #2d3e50 100%)' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-2">👤 {nombreChofer}</h1>
              <p className="text-sm" style={{ color: '#a8e063' }}>Unidad: {unidad || 'N/A'}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-800">Documentos del Chofer</h2>
          <button
